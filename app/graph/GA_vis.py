...
class GA:
  def __init__(self, graph, source, target, population_size=30, generations=200, mutation_rate=0.3, tournament_size=5):
    self.graph = graph
    self.source = source
    self.target = target
  ...
  def run(self):
    ...
    for generation in range(self.generations):
      ...
      formatted_population = [{"route": format_route(route), "distance": 0, "fitness": self.fitness(route)} for route in sorted_population]
      if best_individual:
        formatted_population.append({"route": format_route(best_individual), "distance": 0, "fitness": best_fitness})
      all.append(formatted_population)
      new_population = []
      # best = sorted(self.population, key=lambda x: self.fitness(x))[0]
